...
std::vector<imgDtype> process_window(
    ...
    std::uint32_t y = img_a.height();
    std::uint32_t x = img_a.width();
    std::uint32_t new_x = x / 2 + 1;

    std::uint32_t NM_real = y * x;
    std::uint32_t NM_complex = y * new_x;
    ...
    pocketfft::r2c<imgDtype>(
        ...
        pocketfft::FORWARD, 
        reinterpret_cast<imgDtype*>(&img_a[0]), 
        reinterpret_cast<std::complex<imgDtype>*>(&out_a[0]),
        1.0
    );
    pocketfft::r2c<imgDtype>(
        shape, stride, stride2, axes, 
        pocketfft::FORWARD, 
        reinterpret_cast<imgDtype*>(&img_b[0]), 
        reinterpret_cast<std::complex<imgDtype>*>(&out_b[0]),
        1.0
    );

    out_a = out_b * core::conj(out_a);

    imgDtype scl_fct = 1.0 / NM_real;

    pocketfft::c2r<imgDtype>(
        shape, stride2, stride, axes,
        pocketfft::BACKWARD,
        reinterpret_cast<std::complex<imgDtype>*>(&out_a[0]),
        reinterpret_cast<imgDtype*>(&res[0]),
        scl_fct
        );

    core::swap_quadrants( res );
    
    std::vector<imgDtype> cmatrix( res.pixel_count() );
    std::memcpy(
        cmatrix.data(),
        reinterpret_cast<imgDtype*>(&res[0]),
        res.pixel_count()*sizeof(imgDtype)
    );

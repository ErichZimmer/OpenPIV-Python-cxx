...
std::vector<imgDtype> kernels::gaussian(int kernel_size, imgDtype sigma)
{
    ...
    if (sigma <=0 ){
        sigma = 0.3f*((kernel_size-1)*0.5f - 1) + 0.8f;
    }
    imgDtype s = 2.0f * sigma * sigma;
    ...
    for (i=0 ; i<kernel_size ; i++)
    {
        ...
        {
            kernel[(i*kernel_size)+j] =exp( -0.5f * (pow((i-mean)/sigma, 2.0f) + pow((j-mean)/sigma,2.0f)) ) / (2 * constants::PI * sigma * sigma);

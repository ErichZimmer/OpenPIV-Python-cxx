from numpy.testing import assert_array_almost_equal

import numpy as np
import pytest

from openpiv_cxx import interpolate

try:
    from scipy.ndimage import shift
except ImportError as e:
    raise(e)


def test_taylor_expansion2D_wrong_dim() -> None:
    Z = np.random.rand(256) # should be 2D
    Xq = np.random.rand(256)
    Yq = np.random.rand(256)
    
    with pytest.raises(ValueError):
        out = interpolate.taylor_expansion2D(
            Z,
            Yq,
            Xq
        )

        
def test_taylor_expansion2D_miss_match() -> None:
    Z = np.random.rand(256, 256) 
    Xq = np.random.rand(256, 256)
    Yq = np.random.rand(256, 512)
    
    with pytest.raises(RuntimeError): # error generated by wrapper
        out = interpolate.taylor_expansion2D(
            Z,
            Yq,
            Xq
        )


def test_taylor_expansion2D_wrong_order() -> None:
    Z = np.random.rand(256, 256) 
    Xq = np.random.rand(256, 256)
    Yq = np.random.rand(256, 256)
    
    with pytest.raises(ValueError):
        out = interpolate.taylor_expansion2D(
            Z,
            Yq,
            Xq,
            order = 0
        )


@pytest.mark.parametrize(
    "k", (1, 3, 5, 7)
)
def test_taylor_expansion2D_01(k: int) -> None:
    data = np.array([[4,3,2,1,2,3,4,2,3,6],
                     [5,4,3,2,3,4,5,1,2,8],
                     [0,1,2,3,2,1,0,7,3,6],
                     [6,7,8,9,8,7,6,8,2,5],
                     [6,1,0,2,9,3,2,2,7,9],
                     [9,8,7,6,5,4,3,1,8,3],
                     [1,7,3,7,9,2,6,3,0,2],
                     [6,8,3,5,8,1,7,7,3,5],
                     [4,1,7,3,6,3,6,8,4,6],
                     [0,1,2,3,4,3,2,1,8,4]], dtype = int)

    expected = np.array([[4,4,3,2,1,2,3,4,2,3],
                         [4,4,3,2,1,2,3,4,2,3],
                         [5,5,4,3,2,3,4,5,1,2],
                         [0,0,1,2,3,2,1,0,7,3],
                         [6,6,7,8,9,8,7,6,8,2],
                         [6,6,1,0,2,9,3,2,2,7],
                         [9,9,8,7,6,5,4,3,1,8],
                         [1,1,7,3,7,9,2,6,3,0],
                         [6,6,8,3,5,8,1,7,7,3],
                         [4,4,1,7,3,6,3,6,8,4]], dtype = int)
    
    idy, idx = np.indices(data.shape)
    idy = idy - 1
    idx = idx - 1
    
    result = interpolate.taylor_expansion2D(data, idy, idx, order = k, keep_dtype = True)
    
    assert_array_almost_equal(result, expected)


# k > 1 does not match up with spline interpolation (leading to assert errors), 
# but has similar accuracy on deformation tests.
#@pytest.mark.parametrize(
#    "k", (1, 3, 5, 7) 
#)
def test_taylor_expansion2D_02(k: int = 1) -> None:
    data = np.array([[4,3,2,1,2,3,4],
                     [5,4,3,2,3,4,5],
                     [0,1,2,3,2,1,0],
                     [6,7,8,9,8,7,6],
                     [6,1,0,2,9,3,2],
                     [9,8,7,6,5,4,3]], dtype = float)
    
    expected = shift(data, 0.5, order = k)
    
    idy, idx = np.indices(data.shape)
    
    idy = idy.astype(float)
    idx = idx.astype(float)
    
    idy -= 0.5
    idx -= 0.5
    
    result = interpolate.taylor_expansion2D(data, idy, idx, order = k * 2 - 1, keep_dtype = False)
    
    assert_array_almost_equal(result[1:, 1:], expected[1:, 1:])

    
def test_whittaker2D_wrong_dim() -> None:
    Z = np.random.rand(256) # should be 2D
    Xq = np.random.rand(256)
    Yq = np.random.rand(256)
    
    with pytest.raises(ValueError):
        out = interpolate.whittaker2D(
            Z,
            Yq,
            Xq
        )
    
        
def test_whittaker2D_miss_match() -> None:
    Z = np.random.rand(256, 256) 
    Xq = np.random.rand(256, 256)
    Yq = np.random.rand(256, 512)
    
    with pytest.raises(RuntimeError): # error generated by wrapper
        out = interpolate.whittaker2D(
            Z,
            Yq,
            Xq
        )


def test_whittaker2D_wrong_radius() -> None:
    Z = np.random.rand(256, 256) 
    Xq = np.random.rand(256, 256)
    Yq = np.random.rand(256, 256)
    
    with pytest.raises(ValueError):
        out = interpolate.whittaker2D(
            Z,
            Yq,
            Xq,
            radius = 0
        )
        
        
# testing for whittaker interpolation is done in window deformation
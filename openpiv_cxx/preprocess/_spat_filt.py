...
def intensity_cap(img: ndarray, std_mult: int = 2.0) -> ndarray:
...
def threshold_binarization(img: ndarray, threshold: float = 0.5) -> ndarray:
...
def gaussian_filter(img: ndarray, kernel_size: int = 3, sigma: float = 1.0) -> ndarray:
...
def highpass_filter(
    img: ndarray, kernel_size: int = 3, sigma: float = 1.0, clip_at_zero: bool = True
) -> ndarray:
    ...
    # extract filtered image
    new_img = _gaussian_highpass_filter(buffer1, kernel, bool(clip_at_zero))
...
def variance_normalization_filter(
    ...
    new_img = _local_variance_normalization(
        buffer1, kernel_size, float(sigma1), float(sigma2), bool(clip_at_zero)
    )
...
def contrast_stretch(
    img: ndarray, lower_limit: float = 2.0, upper_limit: float = 98.0
) -> ndarray:

def build_openpivcore(
    ...
    if path.exists(path.join(openpiv_cxx_dir, "build")) != True:
    ...
    if system().lower() == "windows":
        files = list(glob(
            path.normpath(
                path.join(openpiv_cxx_dir, "build/out/Release/*.dll"))
        ))
        files += list(glob(
            path.normpath(
                path.join(openpiv_cxx_dir, "build/openpiv/Release/*.lib"))
        ))
        
    elif system().lower() == "darwin":
        files = list(glob(
            path.normpath(
                path.join(openpiv_cxx_dir, "build/out/*.dylib"))
        ))
        
    elif system().lower() == "linux":
        files = list(glob(
            path.normpath(
                path.join(openpiv_cxx_dir, "build/out/*.so"))
        ))

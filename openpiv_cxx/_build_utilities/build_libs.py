def build_openpivcore(openpiv_cxx_dir: str, _libs_dir: str) -> None:
    ...
    if exists(join(openpiv_cxx_dir, "build")) != True:
        ...
        build_dir = join(openpiv_cxx_dir, "build")
        ...
        build_args = ["cmake", "--build", build_dir, "--clean-first"]
    ...
    if system().lower() == "windows":
        files = list(glob(normpath(join(openpiv_cxx_dir, "build/out/Release/*.dll"))))
        files += list(
            glob(normpath(join(openpiv_cxx_dir, "build/openpiv/Release/*.lib")))
        )

    elif system().lower() == "darwin":
        files = list(glob(normpath(join(openpiv_cxx_dir, "build/openpiv/*.dylib"))))

    elif system().lower() == "linux":
        files = list(glob(normpath(join(openpiv_cxx_dir, "build/openpiv/*.so"))))
